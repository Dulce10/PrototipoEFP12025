...
void cliente::insertar()
{
    ...
    cout << "\t\t\tIngresa Direccion Cliente        : ";
    cin >> direccion;

    cout << "\t\t\tIngresa Correo Cliente        : ";
    cin >> correo;

    cout << "\t\t\tIngresa Estatus Cliente        : ";
    cin >> estatus;
...
void cliente::desplegar() {
    ...
    if (!file) {
        ...
        while (file.read(reinterpret_cast<char*>(&clienteStruct), sizeof(ClienteBin))) {
            ...
            cout << "\t\t\t Apellido Cliente: " << clienteStruct.apellido << endl;
            cout << "\t\t\t Telefono Cliente: " << clienteStruct.telefono << endl;
            cout << "\t\t\t Nit Cliente: " << clienteStruct.nit << endl;
            cout << "\t\t\t Direccion Cliente: " << clienteStruct.direccion << endl;
            cout << "\t\t\t Correo Cliente: " << clienteStruct.correo << endl;
            cout << "\t\t\t Estatus Cliente: " << clienteStruct.estatus << endl;
        }
        file.close();
    }
    system("pause");
    ...
    if (!file) {
        ...
        while (file.read(reinterpret_cast<char*>(&clienteStruct), sizeof(ClienteBin))) {
            if (participant_id != clienteStruct.id) {
                ...
                cin >> direccion;
                cout << "\t\t\tIngrese Correo Cliente: ";
                cin >> correo;
                cout << "\t\t\tIngrese Estatus Cliente: ";
                cin >> estatus;
                tempFile.write(reinterpret_cast<char*>(&clienteStruct), sizeof(ClienteBin));
                ...
                strncpy(clienteStruct.id, id.c_str(), sizeof(clienteStruct.id) - 1);
                strncpy(clienteStruct.nombre, nombre.c_str(), sizeof(clienteStruct.nombre) - 1);
                 strncpy(clienteStruct.apellido, apellido.c_str(), sizeof(clienteStruct.apellido) - 1);
                strncpy(clienteStruct.telefono, telefono.c_str(), sizeof(clienteStruct.telefono) - 1);
                strncpy(clienteStruct.nit, nit.c_str(), sizeof(clienteStruct.nit) - 1);
                 strncpy(clienteStruct.direccion, direccion.c_str(), sizeof(clienteStruct.direccion) - 1);
                  strncpy(clienteStruct.correo, correo.c_str(), sizeof(clienteStruct.correo) - 1);
                   strncpy(clienteStruct.estatus, estatus.c_str(), sizeof(clienteStruct.estatus) - 1);
    ...
    if (!file) {
        ...
        while (file.read(reinterpret_cast<char*>(&clienteStruct), sizeof(ClienteBin))) {
            if (participant_id == clienteStruct.id) {
                ...
                cout << "\t\t\t Telefono Cliente: " << clienteStruct.telefono << endl;
                cout << "\t\t\t Nit Cliente: " << clienteStruct.nit << endl;
                cout << "\t\t\t Nombre Direccion: " << clienteStruct.direccion << endl;
                cout << "\t\t\t Nombre Correo: " << clienteStruct.correo << endl;
                found++;
            }
        }
        if (found == 0) {
